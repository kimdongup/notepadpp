...
- **Column Mode (열 모드...) & Column Editor (열 편집기 `⌥⌘C`)**:
  - **열 모드 안내 (Column Mode...)**: `Edit` -> `Column Mode...` 메뉴를 클릭하면 사각형 열 모드 사용법과 단축키 팁 대화상자가 표시됩니다.
  - **마우스 열 선택**: `⌥` (Option) 키를 누른 채 마우스로 드래그하면 사각형 영역이 선택됩니다.
  - **키보드 열 선택**: `⌥` + `⇧` + 방향키 (`Option + Shift + ↑/↓/←/→`)로 열 블록을 지정합니다.
  - **다중 커서 동시 입력**: 열 모드로 선택된 상태에서 글자를 입력하면 모든 줄에 동시에 텍스트가 삽입됩니다.
  - **열 편집기 (Column Editor `⌥⌘C`)**: 선택된 열에 동일한 텍스트 또는 `1, 2, 3...` 형태의 연속 번호(10진수/16진수/2진수/8진수, 0 채우기 지원)를 일괄 삽입합니다.
- **Convert Case**:
